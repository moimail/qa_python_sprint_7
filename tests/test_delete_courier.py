...
class TestDeleteCourier:
    @allure.title("Успешное уделение курьера.")
    ...
    @allure.title("Попытка уделения курьера без указания id.")
    @allure.description("Получаем код 400")
    def test_delete_courier_without_id(self):
        id_courier = ""
        response = MethodsCourier.delete_courier(id_courier)
        assert response.status_code == 404

    @allure.title("Попытка уделения курьера c указанием несуществующего id.")
    @allure.description(" Получаем код 404")
